  ...
  return (
    <div className="fixed inset-0 z-[300] flex items-center justify-center bg-black/50 p-4">
      <div className="w-full max-w-sm bg-[#0b1017] rounded-2xl p-6 border border-white/10">
        <h3 className="text-xl font-bold text-white mb-3">{titulo || 'Confirmación'}</h3>
        <p className="text-slate-400 text-sm mb-6">{mensaje}</p>
        <div className="flex gap-3">
          <button onClick={onClose} className="flex-1 py-3 rounded-xl bg-white/10 text-white hover:bg-white/20 transition">Cancelar</button>
          <button onClick={onConfirm} disabled={loading} className="flex-1 py-3 rounded-xl bg-cyan-400 text-black font-bold hover:bg-cyan-300 transition disabled:opacity-50">
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/80 backdrop-blur-sm p-6">
      <div className="w-full max-w-lg bg-slate-900 border border-white/10 rounded-3xl p-8">
        <h2 className="text-3xl font-black text-white mb-6">Crear Nómina</h2>
        <div className="grid grid-cols-2 gap-3 mb-6">
          {['mensual', 'quincenal'].map((tipo) => (
            <button key={tipo} onClick={() => h.setTipoPeriodo(tipo)}
              className={`py-4 rounded-2xl font-black capitalize ${h.tipoPeriodo === tipo ? 'bg-cyan-400 text-black' : 'bg-white/5 text-white'}`}>
              {tipo}
            </button>
          ))}
        </div>
        ...
        <div className="flex gap-3">
          <button onClick={onClose} className="flex-1 py-3 bg-white/10 rounded-xl text-white">Cancelar</button>
          <button onClick={async () => { const ok = await h.crearNomina(); if (ok) onClose(); }}
            className="flex-1 py-3 bg-cyan-400 rounded-xl text-black font-bold">Generar</button>
        </div>
      </div>
    </div>
  );
...
export function ModalDetalleEmpleado({ isOpen, onClose, detalle, h }: any) {
  ...
  const [form, setForm] = React.useState({ horas_extra: 0, bonificaciones: 0, comisiones: 0, deducciones: 0 });
  ...
  const guardar = async () => {
    ...
    const payload = {
      ...
      descuentos_legales: Number(form.deducciones)
    };
      ...
      <div className="w-full max-w-2xl bg-[#0b1017] border border-white/10 rounded-3xl overflow-hidden shadow-2xl">
        <div className="flex justify-between items-center px-5 py-4 border-b border-white/5">
          <div>
            ...
            <p className="text-cyan-400 text-xs font-bold uppercase tracking-wider">{detalle.empleados?.nombres} {detalle.empleados?.apellidos}</p>
          </div>
        ...
        <div className="px-5 pb-6 pt-2 flex gap-3">
          {isEditable && (!editMode ? (
            <button onClick={() => setEditMode(true)} className="flex-1 py-3 bg-cyan-400 hover:bg-cyan-500 text-black font-black rounded-xl transition-colors">MODIFICAR VALORES</button>
          ) : (
            <>
              <button onClick={() => setEditMode(false)} className="flex-1 py-3 bg-white/5 text-white font-bold rounded-xl hover:bg-white/10">CANCELAR</button>
              <button onClick={guardar} className="flex-1 py-3 bg-emerald-400 hover:bg-emerald-500 text-black font-black rounded-xl">GUARDAR CAMBIOS</button>
            </>
          ))}
          <button onClick={onClose} className="px-6 py-3 bg-white text-black font-black rounded-xl hover:bg-slate-200">CERRAR</button>
        </div>
      </div>
    </div>
  );
...
export function DrawerEmpleado({ isOpen, onClose, h }: any) {
  ...
  const InputField = ({ label, value, onChange }: any) => (
    <div>
      ...
      <input type="number" className={`${inputS} ${!editable ? 'opacity-50' : ''}`} value={value} disabled={!editable} onChange={(e) => onChange(Number(e.target.value))} />
    </div>
  );
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/70 p-4">
      <div className="w-full max-w-lg bg-[#0b1017] rounded-2xl border border-white/10">
        <div className="p-6 border-b border-white/5">
          <h2 className="text-xl font-black text-white">{empleado ? `${empleado.nombres} ${empleado.apellidos}` : 'Empleado'}</h2>
        </div>
        ...
        <div className="p-6 flex gap-3">
          <button onClick={onClose} className="flex-1 bg-white/10 py-3 rounded-xl">Cerrar</button>
          <button disabled={!editable} onClick={() => { if (editable) h.agregarDetalle(); onClose(); }}
            className="flex-1 bg-cyan-400 text-black font-bold py-3 rounded-xl">Guardar</button>
        </div>
      </div>
    </div>
  );
...
export function PanelDetallesNomina({ isOpen, onClose, nomina, detalles }: any) {
  ...
  const totalPagar = detalles.filter((d: any) => d.periodo === nomina.periodo).reduce((acc: number, d: any) => acc + Number(d.totalPagar || 0), 0);
  const totalBonificaciones = detalles.filter((d: any) => d.periodo === nomina.periodo).reduce((acc: number, d: any) => acc + Number(d.bonificaciones || 0), 0);
  const totalDeducciones = detalles.filter((d: any) => d.periodo === nomina.periodo).reduce((acc: number, d: any) => acc + Number(d.deducciones || 0), 0);
  ...
  return (
    <div className="fixed inset-0 z-[250] flex items-center justify-center bg-black/70 backdrop-blur-md p-4 overflow-y-auto"
      ...
      <div className="w-full max-w-4xl bg-slate-900 border border-blue-500/20 rounded-2xl overflow-hidden shadow-2xl my-8">
        <div className="bg-gradient-to-r from-blue-600/20 to-cyan-600/20 px-6 py-4 border-b border-blue-500/20 flex justify-between items-center">
          ...
          <button onClick={onClose} className="text-white/50 hover:text-white transition-colors text-2xl w-8 h-8 flex items-center justify-center rounded-full hover:bg-white/10">×</button>
        </div>
        <div className="p-5 max-h-[70vh] overflow-y-auto">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-4">
            <InfoCard label="Total Empleados" value={totalEmpleados} color="text-blue-400" />
            <InfoCard label="Total a Pagar" value={quetzal(totalPagar)} color="text-emerald-400" />
            <InfoCard label="Bonificaciones" value={quetzal(totalBonificaciones)} color="text-yellow-400" />
            <InfoCard label="Deducciones" value={quetzal(totalDeducciones)} color="text-rose-400" />
          </div>
          <div className="bg-slate-950 border border-slate-700 rounded-lg p-4 mb-4">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              <div>
                ...
                <p className="text-white text-xs font-semibold mt-1">{new Date(nomina.fecha_inicio).toLocaleDateString('es-GT')}</p>
              </div>
              <div>
                <p className="text-[10px] text-slate-400 font-bold uppercase tracking-wider">Fin</p>
                <p className="text-white text-xs font-semibold mt-1">{new Date(nomina.fecha_fin).toLocaleDateString('es-GT')}</p>
              </div>
              <div>
                <p className="text-[10px] text-slate-400 font-bold uppercase tracking-wider">Creación</p>
                <p className="text-white text-xs font-semibold mt-1">{new Date(nomina.fecha_creacion).toLocaleDateString('es-GT')}</p>
              </div>
              <div>
                ...
                <p className={`font-bold mt-1 text-xs uppercase ${
                  ...
                  nomina.estado?.toLowerCase() === 'cerrada' ? 'text-rose-400' : 'text-yellow-400'
                }`}>{nomina.estado}</p>
              </div>
            </div>
          </div>
          <div className="bg-slate-950 border border-slate-700 rounded-lg overflow-hidden">
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-xs">
                ...
                <tbody>
                  {detalles.filter((d: any) => d.periodo === nomina.periodo).slice(0, 15).map((detalle: any, idx: number) => (
                    <tr key={idx} className="border-b border-slate-700/50 hover:bg-slate-800/30 transition-colors">
                      <td className="px-3 py-2 text-white font-semibold">{detalle.nombres} {detalle.apellidos}</td>
                      <td className="px-3 py-2 text-slate-300">{quetzal(detalle.salarioBase)}</td>
                      <td className="px-3 py-2 text-slate-300">{quetzal(detalle.bonificaciones)}</td>
                      <td className="px-3 py-2 text-slate-300">{quetzal(detalle.deducciones)}</td>
                      <td className="px-3 py-2 text-emerald-400 font-bold">{quetzal(detalle.totalPagar)}</td>
                      <td className="px-3 py-2 text-center">
                        <button onClick={() => h.generarPdfEmpleado(detalle.detalleId)}
        ...
        <div className="bg-slate-950 border-t border-slate-700 px-5 py-3 flex gap-3 justify-end">
          <button onClick={onClose} className="px-4 py-2 bg-white text-black font-bold rounded-lg hover:bg-slate-200 transition-colors text-sm">Cerrar</button>
        </div>
      </div>
    </div>
  );
