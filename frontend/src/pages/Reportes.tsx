...
export default function Reportes() {
  ...
  const retirados = empleados.filter(
    (e) => e.estado?.toLowerCase() === 'retirado',
  ).length;
  ...
  return (
    <div className="min-h-screen space-y-8 bg-slate-950 p-6 text-white">
      ...
      <section className="grid gap-5 md:grid-cols-3 xl:grid-cols-6">
        ...
        <Card
          title="Total Nómina"
          value={`Q${totalNomina.toLocaleString('es-GT', {
        ...
        <Card
          title="Exp. completos"
          value={expedientesCompletos}
          icon="📁"
        />

        <Card
          title="Cumplen"
          value={cumplenContratacion}
          icon="📋"
        />
      </section>
...
<section className="rounded-2xl border border-green-500/20 bg-slate-900 p-6">
  ...
  <div className="mb-4 flex items-center justify-between">
    <h2 className="text-xl font-semibold">
      📄 Todas las Nóminas — PDFs
    </h2>

    <button
      onClick={generarPdfTodasNominas}
        ...
        >
          ...
          <div>
            <p className="font-semibold text-white">
              {n.periodo} — {n.tipo_periodo}
            </p>

            <p className="text-sm text-slate-400">
              {new Date(n.fecha_inicio).toLocaleDateString()} al{' '}
            ...
            <span
              ...
              ${
                n.estado?.toLowerCase() === 'cerrada'
                  ? 'bg-red-500/10 text-red-400'
                  : n.estado?.toLowerCase() === 'procesada'
              ...
              {expedientes.slice(0, 8).map((item) => (
                ...
                >
                  <div className="flex justify-between gap-4">
                    ...
                    <button
                      onClick={() => generarPdfDocumentos(item.empleadoId)}
                      className="rounded-lg border border-blue-500/30 bg-blue-500/10 px-4 py-2 text-sm font-semibold text-blue-400 hover:bg-blue-500/20"
                    >
...
function Card({
  ...
  return (
    <div className="rounded-2xl border border-blue-500/20 bg-slate-900 p-5 shadow-lg">
      ...
      <div className="break-all text-lg font-bold leading-tight text-blue-400">
        {value}
      </div>
