...
export default function DetalleNomina() {
  ...
  const [detalleSeleccionado, setDetalleSeleccionado] = useState<number | undefined>();
  ...
  const isNominaCerrada = estadoActualNormalizado === 'cerrada';

  const estadoOptions =
    estadoActualNormalizado === 'activa'
      ? [{ value: 'activa', label: 'Activa' }, { value: 'cerrada', label: 'Cerrada' }]
      : estadoActualNormalizado === 'cerrada'
      ? [{ value: 'activa', label: 'Activa' }, { value: 'cerrada', label: 'Cerrada' }, { value: 'procesada', label: 'Procesada' }]
      : [{ value: 'procesada', label: 'Procesada' }];

  const detallesFiltrados = h.detallesProcesados.filter((d: any) =>
    `${d.empleados?.nombres || ''} ${d.empleados?.apellidos || ''}`.toLowerCase().includes(busqueda.toLowerCase())
  );
      ...
      <div className="max-w-7xl mx-auto bg-[#0b1017] border border-white/10 rounded-[32px] p-5 md:p-8">
        ...
        <div className="flex flex-col xl:flex-row xl:items-start xl:justify-between gap-8 mb-10">
          <div className="space-y-5">
            ...
            <div>
              <h2 className="text-4xl md:text-5xl font-black tracking-tight">Nómina #{h.nominaId}</h2>
              <p className="text-slate-400 mt-2 text-sm md:text-base">Gestión de colaboradores y pagos</p>
            </div>

            <div className="flex flex-col sm:flex-row sm:items-center gap-3">
              <label className="text-slate-400 text-sm font-semibold">Estado:</label>
              <select
          ...
          <div className="flex flex-wrap gap-3 xl:max-w-[520px]">
            ...
            <div className="relative flex-1 min-w-[180px]">
              <button
                ...
                disabled={!isNominaEditable || h.loading}
                className={`w-full px-6 py-4 transition font-black rounded-2xl border ${showDeptPanel ? 'bg-cyan-400 text-black' : 'bg-[#05070a] text-white border-white/10'}`}
              >
                Por departamento
              </button>
              {showDeptPanel && (
                <div className="absolute top-full right-0 mt-3 w-full bg-slate-900 border border-white/10 rounded-2xl shadow-2xl z-20 overflow-hidden">
                  {h.departamentos.map((d: any) => (
                    <button
                      key={d.id}
                      onClick={() => { h.agregarEmpleadosPorDepartamento(Number(d.id)); setShowDeptPanel(false); }}
            ...
            <div className="relative flex-1 min-w-[180px]">
              <button
                ...
                disabled={!isNominaEditable || h.loading}
                className={`w-full px-6 py-4 transition font-black rounded-2xl border ${showPuestoPanel ? 'bg-cyan-400 text-black' : 'bg-[#05070a] text-white border-white/10'}`}
              >
                Por puesto
              </button>
              {showPuestoPanel && (
                <div className="absolute top-full right-0 mt-3 w-full bg-slate-900 border border-white/10 rounded-2xl shadow-2xl z-20 overflow-hidden">
                  {h.puestos.map((p: any) => (
                    <button
                      key={p.id}
                      onClick={() => { h.agregarEmpleadosPorPuesto(Number(p.id)); setShowPuestoPanel(false); }}
                      className="w-full text-left px-5 py-4 hover:bg-cyan-400 hover:text-black transition text-sm font-bold border-b border-white/5"
                    >
                      {p.nombre}
                    </button>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-5 mb-10">
          <div className="bg-[#05070a] border border-white/10 rounded-3xl p-6">
            ...
            <p className="text-4xl font-black mt-3">{h.resumen.empleados}</p>
          </div>
          <div className="bg-[#05070a] border border-white/10 rounded-3xl p-6">
            <p className="text-slate-500 text-sm font-bold uppercase tracking-wider">Total a pagar</p>
            <p className="text-4xl font-black mt-3 text-cyan-400 break-words">{quetzal(h.resumen.total)}</p>
          </div>
          <div className="bg-[#05070a] border border-white/10 rounded-3xl p-6">
            <p className="text-slate-500 text-sm font-bold uppercase tracking-wider">Estado</p>
            <p className="text-2xl font-black mt-3 uppercase">{estadoActualNormalizado}</p>
          </div>
        </div>
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-5 mb-4">
          <div>
            <h3 className="text-2xl font-black">Detalle de Pagos</h3>
            <p className="text-slate-500 text-sm mt-1">Empleados agregados a esta nómina</p>
          </div>
          <button
        ...
        <div className="mb-6">
          <input
            ...
            className="w-full rounded-2xl border border-white/10 bg-[#05070a] px-5 py-3 text-white outline-none focus:border-cyan-400 placeholder:text-slate-600"
          />
        </div>
        ...
        <div className="overflow-x-auto border border-white/5 rounded-3xl">
          <table className="w-full min-w-[1100px] text-left">
            <thead className="bg-white/[0.03] text-slate-400 text-xs uppercase">
              <tr>
                ...
                <th className="py-5">Salario Base</th>
                <th className="py-5">Extras</th>
                <th className="py-5">Deducciones</th>
              ...
              {detallesFiltrados.map((d: any) => (
                <tr key={d.id} className="hover:bg-white/[0.03] transition">
                  <td className="px-6 py-6">
                    ...
                    >
                      <div className="font-black text-base">{d.empleados?.nombres}</div>
                      <div className="text-sm text-slate-500 mt-1">{d.empleados?.apellidos}</div>
                    </button>
                  </td>
                  <td className="py-6 font-semibold text-base">{quetzal(d.salario_base)}</td>
                  <td className="py-6">
                    <div className="text-emerald-400 font-black text-lg">{quetzal(d.total_extras)}</div>
                    <div className="text-xs text-slate-500 mt-1">Bonos y extras</div>
                  </td>
                  <td className="py-6">
                    <div className="text-red-400 font-black text-lg">{quetzal(d.deducciones)}</div>
                    <div className="text-xs text-slate-500 mt-1">IGSS y descuentos</div>
                  </td>
                  <td className="py-6">
                    <div className="text-cyan-400 font-black text-2xl">{quetzal(d.salario_final)}</div>
                  </td>
                  <td className="py-6 px-6">
                    <div className="flex flex-col gap-2 items-end">
                      ...
                      <button
                        onClick={() => { setDetalleSeleccionado(d.id); setHistorialOpen(true); }}
          ...
          {detallesFiltrados.length === 0 && (
            <div className="text-center py-24 text-slate-600 text-lg">
              {busqueda ? 'No se encontraron empleados con ese nombre' : 'No hay empleados vinculados a esta nómina'}
            </div>
          )}
        </div>
