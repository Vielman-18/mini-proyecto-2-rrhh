...
export const empleadoSchema = z.object({
  ...
  dpi: z
    ...
    .refine((val) => !/\s/.test(val), { message: 'El DPI no puede contener espacios' }),
  ...
  telefono: z
    ...
    .refine((val) => !/\s/.test(val), { message: 'El teléfono no puede contener espacios' }),

  email: z
    .string()
    .min(1, 'El email es obligatorio')
    .email('El email no es válido')
    .refine((val) => !/\s/.test(val), { message: 'El email no puede contener espacios' }),

  salario: z
    .union([z.string(), z.number()])
    .refine((val) => Number(val) > 0, { message: 'El salario debe ser mayor a 0' }),
